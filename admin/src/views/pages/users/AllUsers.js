...
import { CAlert, CAvatar, CButton, CCard, CCardBody, CCardHeader, CModal, CModalBody, CModalFooter, CModalHeader, CModalTitle, CSpinner } from '@coreui/react'
...
const AllUsers = () => {
    ...
    useEffect(() => {
        ...
        if (users) {
            setRows(users.map(user => ({
                ...
                email: user.email,
                avatar: <CAvatar shape="rounded-circle" color="secondary" textColor="white" src={user.avatar.url !== "no avatar" && user.avatar.url}>{user.avatar.url === "no avatar" && user.name.slice(0, 1)}</CAvatar>,
                actions: <>
                            <CButton color="info" variant="ghost" size="sm" onClick={() => history.push(`/admin/users/${user._id}`)}>
        ...
        return (
          ...
          >
            ...
            <CModalFooter>
              ...
              <CButton disabled={loadingUser} color="danger" onClick={() => handleDeleteUser(toDelete)}>Delete</CButton>
            </CModalFooter>
          </CModal>
        )
    }
    ...
    return (
        ...
        <CCard className="mb-4">
            ...
            <CCardBody>
              <ConfirmDelete />
              {!loading && users ? <Table columns={columns} rows={rows} scrollY /> 
