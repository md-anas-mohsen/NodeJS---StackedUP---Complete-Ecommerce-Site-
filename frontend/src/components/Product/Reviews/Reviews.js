...
export default function Reviews({openState, reviews, productName}) {
  ...
  useEffect(() => {
    if(reviews && user) {
        setUserReview(reviews.find(review => review.user === user?._id));
    }
  }, [reviews, user]);
  ...
  return (
      <Dialog fullScreen open={openReviews} PaperProps={{style: {backgroundColor: MaximumYellowRed}}} onClose={handleClose} TransitionComponent={Transition}>
        ...
        <div className={classes.layout}>
            {userReview && <ReviewCard userReview key={userReview?._id} review={userReview} />}
            {reviews?.map(review => (
