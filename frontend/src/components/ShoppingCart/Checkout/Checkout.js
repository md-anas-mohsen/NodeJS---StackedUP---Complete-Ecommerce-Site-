...
import { Grid, Paper, Step, StepLabel, Stepper, TextField, Select, MenuItem, FormControl, InputLabel, Avatar, ThemeProvider, CircularProgress } from '@material-ui/core';
import { AppContext, CheckoutContext } from '../../../context/AppContext';
import { countries } from 'countries-list';
import { clearCart, processPayment, saveShippingInfo } from '../../../actions/cartActions';
...
function Review({ params, cardDetails }) {
    ...
    const products = cartItems.map(item => ({
      ...
      name: item.name,
      price: item.price,
      desc:  item.quantity > 1 ? `${item.quantity} units` : `${item.quantity} unit`,
      qty: item.quantity
    }));
    const addresses = [
    ...
    // const payments = [
    //   { name: 'Card type', detail: cardDetails?.brand },
    //   { name: 'Card holder', detail: cardDetails?.customer },
    //   { name: 'Card number', detail: 'xxxx-xxxx-xxxx-1234' },
    //   { name: 'Expiry date', detail: '04/2024' },
    // ];
    const basePrice = cartItems.reduce((acc, item) => acc + item.quantity * item.price, 0);
      ...
      <React.Fragment>
        <List disablePadding>
          {products.map((product) => (
            <ListItem className={classes.listItem} key={product.productID}>
              ...
              <Link className="product__info__link" to={`/product/${product.productID}`}>
                <ListItemText style={{width: "85%"}} primary={product.name} secondary={product.desc} />
              </Link>
              <Typography variant="body2">${Number(product.price*product.qty).toFixed(2)}</Typography>
            </ListItem>
          ))}
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            ...
            <Typography gutterBottom><strong>Name: </strong>{user.name}</Typography>
            <Typography gutterBottom><strong>Phone: </strong>{shippingInfo.phoneNo}</Typography>
          ...
          <Button
            ...
            onClick={placeOrder}
            disabled={loading}
          >{ loading ? <CircularProgress /> : `Place Order` }</Button>
        </Grid>
      </React.Fragment>
    );
  }
    ...
    return (
      <React.Fragment>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={6}>
            <TextField variant="filled" id="state" name="state" label="State/Province/Region" fullWidth 
            value={province} 
    ...
    return (
      <React.Fragment>
        <div style={{display: step === 2 ? "block" : "none"}}>
          <Review 
            params={[paymentData, stripe, elements, CardNumberElement, cardName, user.email]}
...
export default function Checkout({openState}) {
  ...
  const { order, orderError, loadingOrder } = useSelector(state => state.newOrder);
  const { cartItems, shippingInfo, result, error, loading } = useSelector(state => state.cart);
  ...
  const [orderInfo, setOrderInfo] = useState(sessionStorage.getItem("orderInfo") || {});

  useEffect(() => {
    if(Object.keys(orderInfo).length === 0 && orderInfo.constructor === Object) {
      setActiveStep(0);
    }
  }, [orderInfo]);
  ...
  }, [error, result, setAlert, dispatch, orderError, cartItems, orderInfo, shippingInfo, activeStep]);
  ...
  const handleNext = () => {
    if(activeStep === 0) {
      dispatch(saveShippingInfo({address, phoneNo, city, postalCode, province, country}));
  ...
  return (
      ...
      <Dialog fullScreen open={openCheckout} PaperProps={{style: {backgroundColor: MaximumYellowRed}}} onClose={handleClose} TransitionComponent={Transition}>
        ...
        <Paper className={classes.paper} elevation={5}>
          ...
          <React.Fragment>
            {activeStep === steps.length ? (
              <React.Fragment>
                <center>
                  <object aria-label="order confirmed" type="image/svg+xml" data="images/order_confirmed.svg" width="300px" />
                </center>
              ...
              <React.Fragment>
                <div style={{padding: "20px 0"}}>
                  <div style={{display: activeStep === 0 ? "block" : "none"}}><AddressForm /></div>
                  <div style={{display: activeStep === 1 || activeStep === 2 ? "block" : "none"}}>
                    {stripeApiKey && 
                    <Elements stripe={loadStripe(stripeApiKey)}>
                      <PaymentForm step={activeStep} orderInfo={orderInfo} />
                    </Elements>}
                  </div>
                </div>
                <div className={classes.buttons}>
                  ...
                  {activeStep !== steps.length - 1 && 
                    <Button
                      ...
                      onClick={handleNext}
                      className={classes.button}
                      disabled={error || loading || loadingOrder || orderError}
                    >
                      Next
                    </Button>
                  }
