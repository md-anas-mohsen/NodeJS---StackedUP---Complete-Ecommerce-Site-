...
import { Info, MaximumRed, Orange, Success, Warning } from '../../Misc/Colors/Colors';
import { Skeleton } from '@material-ui/lab';

export const Status = ({status, size}) => {
    const Icon = () => {
        if(status === "Processing") return <InfoIcon fontSize="small" style={size && {fontSize: size === "large" && "25px"}} />
        if(status === "Delivered" || status === "Paid") return <CheckCircleIcon fontSize="small" style={size && {fontSize: size === "large" && "25px"}} />
        if(status === "Not Paid") return  <WarningIcon fontSize="small" style={size && {fontSize: size === "large" && "25px"}} />
        if(status === "In Transit") return  <LocalShippingIcon fontSize="small" style={size && {fontSize: size === "large" && "25px"}} />
    }
    return (
        <div
        style={{
            backgroundColor: status === "Processing" ? Info 
                            :status === "Delivered" || status === "Paid" ? Success
